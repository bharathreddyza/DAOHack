    ...
    <div className='my-10'>
        ...
        <div className='overflow-x-auto flex flex-nowrap mx-10  '>
            {dao.topTokenHolders.map((holder)=> (
                <div onClick={()=>navigate(`/users/${holder.contract_address}`)} className=' cursor-pointer hover:bg-slate-400 flex p-2 m-2 bg-gray-200 rounded-md' key={holder.contract_address}>
                            <UserIcon className='w-8 h-8 p-1 bg-purple-500  rounded-full text-white'/>
                            <div className='text-left'>
                                <h1>{ ((holder.balance/holder.total_supply)* 100) } %</h1>
